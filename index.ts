...
app.get("/api/me", requireAuth as express.RequestHandler, (req, res) =>
  getMe(req as AuthRequest, res),
);
app.post("/api/showcase", requireAuth as express.RequestHandler, (req, res) =>
  upsertShowcase(req as AuthRequest, res),
);
app.delete(
  "/api/showcase/:id",
  requireAuth as express.RequestHandler,
  (req, res) => deleteShowcase(req as AuthRequest, res),
);
...
function clearAuthFolder(): void {
  ...
  try {
    ...
    logger.info(
      `🗑️ Cleared ${cleared}/${files.length} auth files. Bot akan restart...`,
    );
  } catch (err) {
...
async function processMessage(msg: MessageEvent): Promise<void> {
  try {
    ...
    if (dbResult.status === "rejected") {
      logger.error("❌ Failed to save message to database", dbResult.reason);
    }
...
async function startBot(): Promise<void> {
  ...
  try {
    ...
    if (state.creds && (state.creds as any).me?.id && !state.creds.registered) {
      logger.info(
        "🛠️  Session found but unregistered. Forcing 'registered' to true...",
      );
    ...
    sock.ev.on("connection.update", (update) => {
      ...
      if (qr && !pairingCodeRequested) {
        ...
        if (
          ...
          logger.debug(
            "Auth folder exists with registered session; skipping pairing code request.",
          );
          ...
          setTimeout(async () => {
            try {
              ...
              logger.info(
                "3. Pilih 'Tautkan dengan Nomor Telepon Saja' (di bawah).",
              );
      ...
      if (connection === "close") {
        ...
        if (errorCode === 440) {
          logger.error("❌ ERROR 440: Session already logged in elsewhere!");
          logger.info("Clearing 'auth_info_baileys' folder and restarting...");
          clearAuthFolder();
          process.exit(1);
        }
        ...
        if (shouldReconnect) {
          ...
          logger.warn("🚪 Bot logged out. Please run again to scan QR.");
          botState.isConnecting = false;
        }
    ...
    sock.ev.on("messages.upsert", async (m: any) => {
      try {
        ...
        if (senderJid !== ownerJid && senderJidAlt !== ownerJid) {
          logger.warn(
            `⚠️ Ignored message from non-owner: ${senderJid} (Alt: ${senderJidAlt})`,
          );
          return;
        }
        ...
        logger.info(`📨 Message received: "${content.substring(0, 50)}..."`);
